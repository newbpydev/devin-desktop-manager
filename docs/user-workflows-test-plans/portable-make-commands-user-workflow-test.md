...
| PMC-WF-017 | Coverage ordinary failure | Seed valid coverage, fail Bashcov/Bats/parser/threshold/publication one point at a time, then run successfully. | Every failed run preserves prior output; successful run publishes one fresh 90%+ result and runs the suite once. | PMC-U5-R01, PMC-U5-R02, PMC-U5-R06 |
